...
principal argument for those operations instead.
  ...
  s" complex.fs" included  \ defines PRINCIPAL-ARG  
...
t{ 1E1 2E1 3E1 -frot -> 3E1 1E1 2E1 }t
t{ 1E1 2E1     fnip  -> 2E1         }t
t{ 1E1 2E1     ftuck -> 2E1 1E1 2E1 }t
...
set-exact
t{  11E   3E fcopysign ->  11E }t
t{  -7E   5E fcopysign ->   7E }t
t{   5E  -7E fcopysign ->  -5E }t
t{  -3E -11E fcopysign ->  -3E }t
[THEN]
...
set-exact
t{ 0+i0      zdrop ->                }t
t{ 1+i0      zdup  -> 1+i0 1+i0      }t
t{ 0+i0 1+i0 zswap -> 1+i0 0+i0      }t
t{ 0+i0 1+i0 zover -> 0+i0 1+i0 0+i0 }t
t{ 0+i0 1+i0 znip  -> 1+i0           }t
t{ 0+i0 1+i0 ztuck -> 1+i0 0+i0 1+i0 }t
t{ 0+i0 1+i0 0+i1  zrot -> 1+i0 0+i1 0+i0 }t
t{ 0+i0 1+i0 0+i1 -zrot -> 0+i1 0+i0 1+i0 }t
...
t{ 1+i0  real  -> 1E }t
t{ 0+i1  imag  -> 1E }t
t{ 1E 2E conjg -> 1E -2E }t

set-near  \ true also works in pfe
t{ 1E 2E 3E    z*f ->  3E  6E }t
t{ 3E 6E 3E    z/f ->  1E  2E }t
t{ 1E 2E 3E 4E z*  -> -5E 10E }t
t{ 1E 2E 1+i0  z*  ->  1E  2E }t
t{ 1E 2E 0+i1  z*  -> -2E  1E }t
t{ 1E 1E 3E 4E z/  ->  7E 25E f/ -1E 25E f/ }t
t{ 1E 1E 4E 3E z/  ->  7E 25E f/  1E 25E f/ }t
t{ 1E 2E 3E 4E z+  ->  4E  6E }t
t{ 1E 2E 3E 4E z-  -> -2E -2E }t
...
set-exact
t{   1E   -2E  -z  ->  -1E    2E  }t
t{  40E1 -20E1 z2* ->  80E1 -40E1 }t
t{ -40E1  20E1 z2* -> -80E1  40E1 }t
t{  50E1 -30E1 z2/ ->  25E1 -15E1 }t
t{ -50E1  30E1 z2/ -> -25E1  15E1 }t
t{  40E1 -20E1 i*  ->  20E1  40E1 }t
t{ -40E1  20E1 i*  -> -20E1 -40E1 }t
t{  40E1 -20E1 -i* -> -20E1 -40E1 }t
t{ -40E1  20E1 -i* ->  20E1  40E1 }t
...
set-exact
t{ 1E 2E 3E x+ ->  4E 2E }t
t{ 1E 2E 3E x- -> -2E 2E }t
t{ 1E 2E 4E y+ ->  1E  6E }t
t{ 1E 2E 4E y- ->  1E -2E }t

[DEFINED] i*f/z [IF]
testing  Z*>REAL Z*>IMAG  Z*F Z/F F*Z F/Z Z*I*F -I*Z/F I*FZ* I*F/Z

set-exact
t{ 1E 2E 3E 4E z*>real -> -5E }t
t{ 1E 2E 3E 4E z*>imag -> 10E }t
t{ -1E  2E 3E z*f -> -1E  2E  3E 0E z* }t
t{ -3E -6E 3E z/f -> -3E -6E  3E 0E z/ }t
t{  3E  1E 2E f*z ->  3E  0E  1E 2E z* }t
set-near  \ pfe works with exact
t{  3E  6E 3E f/z  ->  3E  0E  6E 3E  z/ }t
set-exact
t{ -1E  2E 3E  z*i*f -> -1E  2E  0E 3E z* }t
t{ -3E -6E 3E -i*z/f -> -3E -6E  0E 3E z/ }t
t{  3E  1E 2E  i*f*z ->  0E  3E  1E 2E z* }t
t{  3E  6E 3E  i*f/z ->  0E  3E  6E 3E z/ }t
[THEN]
...
t{ -3E  4E |z|^2 -> 25E }t
t{  3E -4E |z|^2 -> 25E }t
t{  3E  4E 1/z   ->  3E  25E f/ -4E 25E f/ }t
t{ -3E  4E 1/z   -> -3E  25E f/ -4E 25E f/ }t
t{  3E -4E 1/z   -> -3E -25E f/  4E 25E f/ }t
t{  1+i0   1/z   ->  1+i0 }t
t{  3E  4E z^2   -> -7E  24E }t
t{ -3E  4E z^2   -> -7E -24E }t
t{  3E -4E z^2   -> -7E -24E }t
t{ -3E -4E z^2   -> -7E  24E }t
...
set-near
t{  2E  2E   arg -> pi/4   }t
t{  3E -3E   arg -> 315arg }t
set-exact
t{ -rt2  rt2 arg -> 3pi/4  }t
t{ -rt2 -rt2 arg -> 225arg }t
...
t{  0+i0     >polar -> 0+i0     }t
t{  1+i0     >polar -> 1+i0     }t
t{  0+i1     >polar -> 1E pi/2   }t
t{  0-i1     >polar -> 1E 270arg }t
t{ -1+i0     >polar -> 1E pi     }t
set-near
t{  2E   2E  >polar -> 2rt2 pi/4   }t
t{  3E  -3E  >polar -> 3rt2 315arg }t
t{ -rt2  rt2 >polar -> 2E   3pi/4  }t
t{ -rt2 -rt2 >polar -> 2E   225arg }t

set-exact
t{ 0+i0        polar> ->  0+i0 }t
t{ 1+i0        polar> ->  1+i0 }t
set-near
t{ 1E    pi/2  polar> ->  0+i1 }t \ Re(f) = -6.123234E-17
t{ 1E   -pi/2  polar> ->  0-i1 }t \ Re(f) = 6.123234E-17
t{ 1E    pi    polar> -> -1+i0 }t \ Im(f) = 1.224647E-16
t{ 2rt2  pi/4  polar> ->  2E   2E  }t
t{ 3rt2 -pi/4  polar> ->  3E  -3E  }t
t{ 2E    3pi/4 polar> -> -rt2  rt2 }t
t{ 2E   -3pi/4 polar> -> -rt2 -rt2 }t
...
t{ 0+i0 zsqrt -> 0+i0 }t
t{  2E   0E  ?gauge }t
t{ -2E   0E  ?gauge }t \ Re(g) = 8.659561E-17
t{  0E   2E  ?gauge }t
t{  0E  -2E  ?gauge }t
t{  rt2  rt2 ?gauge }t
t{  rt2 -rt2 ?gauge }t
...
set-exact
t{  1+i0     zln -> 0+i0      }t
t{  0+i1     zln -> 0E pi/2   }t
t{  0-i1     zln -> 0E 270arg }t
t{ -1+i0     zln -> 0E pi     }t
set-near
t{ -2E   0E  zln -> ln2 pi     }t
t{  rt2  rt2 zln -> ln2 pi/4   }t
t{  rt2 -rt2 zln -> ln2 315arg }t
t{ -rt2  rt2 zln -> ln2 3pi/4  }t
t{ -rt2 -rt2 zln -> ln2 225arg }t
...
set-exact
t{  0+i0       zexp ->  1+i0 }t
t{  ln2  0E    zexp ->  2E   0E }t
set-near
t{ -ln2  0E    zexp ->  0.5E 0E }t
t{  0E   pi    zexp -> -1+i0    }t \ Im(f) = 1.224647E-16
t{  0E   pi/2  zexp ->  0+i1    }t \ Re(f) =6.123234E-17
t{  0E  -pi/2  zexp ->  0+i1 conjg }t \ Re(f) = 6.123234E-17
t{  0E   pi/4  zexp ->  1/rt2   1/rt2  }t
t{  0E  -pi/4  zexp ->  1/rt2  -1/rt2  }t
t{  0E   3pi/4 zexp -> -1/rt2   1/rt2  }t
t{  0E  -3pi/4 zexp -> -1/rt2  -1/rt2  }t
t{  ln2  pi/4  zexp ->  rt2     rt2    }t
t{  ln2 -pi/4  zexp ->  rt2    -rt2    }t
t{ -ln2  3pi/4 zexp -> -1/2rt2  1/2rt2 }t
t{ -ln2 -3pi/4 zexp -> -1/2rt2 -1/2rt2 }t

set-exact
t{  1+i0     0+i0  z^ ->  1+i0 }t
t{ -1+i0     0+i0  z^ ->  1+i0 }t
t{  0+i1     0+i0  z^ ->  1+i0 }t
t{  0-i1     0+i0  z^ ->  1+i0 }t
t{  rt2  rt2 0+i0  z^ ->  1+i0 }t
t{  rt2 -rt2 0+i0  z^ ->  1+i0 }t
t{ -rt2  rt2 0+i0  z^ ->  1+i0 }t
t{ -rt2 -rt2 0+i0  z^ ->  1+i0 }t
...
set-near
t{  1+i0     ?gauge }t
t{ -1+i0     ?gauge }t \ Im(f) = 1.224647E-16
t{  0+i1     ?gauge }t \ Re(f) = 6.123234E-17
t{  0-i1     ?gauge }t \ Re(f) = 6.123234E-17
t{  rt2  rt2 ?gauge }t
t{  rt2 -rt2 ?gauge }t
t{ -rt2  rt2 ?gauge }t
...
t{  0-i1     ?gauge }t
t{  rt2  rt2 ?gauge }t
t{  rt2 -rt2 ?gauge }t \ pfe 1
t{ -rt2  rt2 ?gauge }t
t{ -rt2 -rt2 ?gauge }t \ pfe 2
...
t{  1+i0     zcosh -> zch1 }t
t{ -1+i0     zcosh -> zch1 }t
t{  0E  pi/2 zcosh -> 0+i0 }t \ Re(f) = 6.123234E-17
t{  0E -pi/2 zcosh -> 0+i0 }t \ Re(f) = 6.123234E-17
t{  1E  pi/4 zcosh -> zC1  }t
t{  1E -pi/4 zcosh -> zC2  }t
t{ -1E  pi/4 zcosh -> zC2  }t
t{ -1E -pi/4 zcosh -> zC1  }t
...
set-near
t{  1+i0     zsinh -> zsh1 }t
t{ -1+i0     zsinh -> sh1 fnegate 0E }t
t{  0E  pi/2 zsinh -> 0+i1 }t
t{  0E -pi/2 zsinh -> 0+i1 conjg }t
t{  1E  pi/4 zsinh -> zC3  }t
t{  1E -pi/4 zsinh -> zC4  }t
...
t{  0E  pi/4 ztanh -> 0+i1 }t
t{  0E -pi/4 ztanh -> 0+i1 conjg }t
t{  1E  pi/4 ztanh -> ztanhA }t
t{  1E -pi/4 ztanh -> ztanhB }t
t{ -1E  pi/4 ztanh -> ztanhB -z  }t
t{ -1E -pi/4 ztanh -> ztanhA -z  }t

set-near
t{  1+i0     zcoth -> zcth1 }t
t{ -1+i0     zcoth -> zcth1 -z  }t
t{  0E  pi/4 zcoth -> 0+i1 conjg }t
t{  0E -pi/4 zcoth -> 0+i1 }t
t{  1E  pi/4 zcoth -> zcothA }t
t{  1E -pi/4 zcoth -> zcothB }t
...
t{ 1+i0 ?inverse }t
t{ zch1 ?inverse }t
t{ 0+i0 ?inverse }t \ Re(f^-1 f) = 6.123234E-17, Im((f^-1 f) = 4.440892E-16
t{ zC1  ?inverse }t
t{ zC2  ?inverse }t
...
t{ 1+i0 ?gauge }t
t{ zch1 ?gauge }t
t{ 0+i0 ?gauge }t \ Re(g) = 2.220446E-16
t{ zC1  ?gauge }t
t{ zC2  ?gauge }t
...
t{ ztanhA     ?inverse }t
t{ ztanhB     ?inverse }t
t{ ztanhB -z  ?inverse }t
t{ ztanhA -z  ?inverse }t
...
t{ zcothA     ?inverse }t
t{ zcothB     ?inverse }t
t{ zcothB -z  ?inverse }t
t{ zcothA -z  ?inverse }t
...
t{ zC3 ?inverse }t
t{ zC4 ?inverse }t
t{ zC1 gasin zsin -> zC1 }t
t{ zC1 gatan ztan -> zC1 }t
t{ zC1 gacot zcot -> zC1 }t

' zacos is func
' gacos is gauge
\ stay off the cut |x| >= 1
set-near
t{  0E   0E    ?gauge }t \ Im(g) = -4.440892E-16
t{  pi/4  0E   ?gauge }t \ Im(g) = 2.220446E-16
t{ -pi/4  0E   ?gauge }t \ Im(g) = 2.220446E-16
t{  0E    pi/4 ?gauge }t
t{  0E   -pi/4 ?gauge }t
t{  0E    pi/2 ?gauge }t
...
set-near
t{  0E   0E    ?gauge }t
t{  pi/4  0E   ?gauge }t \ Im(g) = -1.348379E-17
t{ -pi/4  0E   ?gauge }t \ Im(g) = -1.348379E-17
t{  0E    pi/4 ?gauge }t
t{  0E   -pi/4 ?gauge }t
t{  0E    pi/2 ?gauge }t
