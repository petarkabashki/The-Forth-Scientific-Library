...
\ attempts to determine whether floating-point support is present, and if so,
\ whether there is a separate floating-point stack, and behave accordingly.
\ The CONSTANTs  HAS-FLOATING  and  HAS-FLOATING-STACK  contain the results of
\ its efforts, so the behavior of the code can be modifiesd by the user if necessary.

\ Then there are the perennial issues of floating point value comparisons.
\ Exact equality is specified by  SET-EXACT  (the default).
\ If approximate equality tests are desired, execute  SET-NEAR .
\ Then the FVARIABLEs  REL-NEAR  (default 1E-12) and  ABS-NEAR  (default 0E)
\ contain the values to be used in comparisons by the (internal) word FNEARLY= .

\ When there is not a separate floating point stack, it is necessary to identify
\ which stack items are floating point quantities.  This can be done by replacing
\ the closing  }T  with a version that specifies this, such as  RRXR}T  which
\ identifies the stack picture  ( r r x r ).  The code provides such words for 
\ all combinations of R and X with up to four stack items.  They can be used with
\ either an integrated or separate floating point stacks. Adding more if you
\ need them is straightforward; see the examples in the source.
\ Here is an example which also illustrates controlling the precision of comparisons:
\   SET-NEAR
\   1E-6 REL-NEAR F!
\   T{  S" 3.14159E" >FLOAT  ->  -1E FACOS TRUE  RX}T

\ The word ERROR is now vectored, so that its action can be changed 
\ by the user (for example, to add a counter for the number of errors).
...
\ The file defines some 70 words in all, but in most cases only the ones 
\ mentioned above will be needed for successful testing.
